<template>
  <div class="wMeterEdit">
    ...
    <div v-if="wMeterEditVisible">
      <wMeterEditAdd ref="wMeterEditAdd" :tableEdit="tableEdit"></wMeterEditAdd>
    </div>
    ...
    <div v-else class="wMeterEditCon">
      ...
      <div class="kl-table">
        <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="histroyData.list" @cell-click="cellClick" class="wMeterEdit-table">
          ...
          <el-table-column prop="fixValue" min-width="120" label="修改原因">
          </el-table-column>
...
export default {
    ...
    return {
      crumbsData: {  //面包屑
        titleList: [
          ...
          { title: '水表资料修改', method: () => { window.histroy.back() } }
        ],
  ...
  mounted() {
    ...
    this.$changeTable(this, '.wMeterEdit', ['.wMeterEdit .toolbar', '.wMeterEdit .block', '.wMeterEdit .bread-contain'])
  },
  methods: {
    init() {
...
#app .wMeterEdit .bread-contain #crumbs{
  width: 50%;
}
#app .wMeterEdit .bread-contain .bread-contain-right{
  width: 45%;
}
</style>
