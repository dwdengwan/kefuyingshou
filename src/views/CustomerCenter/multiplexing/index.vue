...
<template>
  <!-- <div class="multiplexing">
    ...
    <multiplexingEdit ref="childmultiplexing"></multiplexingEdit>
  </div> -->
  <div class="multiplexing">
    <!-- 弹出表单选择用户 -->
    ...
    <span v-if="!multiplexingVisible && !multiplexingAddVisible">
      ...
      <div class="kl-table">
        <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="histroyData.list" class="change-tables-table">
          ...
          <el-table-column prop="fixValue" min-width="80" label="是否复表">
          </el-table-column>

          <el-table-column prop="flux" min-width="80" label="起始抄码">
          </el-table-column>

          <el-table-column prop="fixValue" min-width="80" label="复用说明">
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      crumbsData: {  //面包屑
        titleList: [
          { title: '客户中心', path: '/multiplexing' },
          { title: '复用', method: () => { window.histroy.back() } }
        ],
  ...
  methods: {
    ...
    //弹出框
    submit(formName) {	//提交
      this.$refs.childmultiplexing.submit()
      // this.multiplexingVisible = false;
    },
    edit() {
