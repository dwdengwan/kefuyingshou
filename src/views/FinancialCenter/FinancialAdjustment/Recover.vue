...
<template>
  ...
  <div class="Recover">
    <!-- 弹出表单编辑 -->
    <!-- <el-dialog :title="RecoverName" :close-on-click-modal="false" :visible.sync="RecoverVisible" class="addDialog" @close="closeDialog" >
            <RecoverEdit
                ref="childRecover" 
            ></RecoverEdit>
    ...
    <div v-else>
      ...
      <div class="kl-table">
        <el-table v-if="tableShow" :max-height="maxHeight" stripe border :data="histroyData.list" class="change-tables-table">
          ...
          <el-table-column prop="factoryName" min-width="140" label="用户名称">
          </el-table-column>

          <el-table-column prop="fixValue" min-width="150" label="用户地址">
          </el-table-column>

          <el-table-column prop="businessBranch" min-width="100" label="营业网点">
          </el-table-column>
          ...
          <el-table-column prop="position" min-width="100" label="追缴日期">
          </el-table-column>

          <el-table-column prop="useWaterType" min-width="100" label="用水类型">
          </el-table-column>

          <el-table-column prop="costName" min-width="100" label="费用名称">
          </el-table-column>
          
          <el-table-column prop="model" min-width="80" label="总金额">
          </el-table-column>

          <el-table-column prop="woker" min-width="100" label="收费员">
          </el-table-column>

          <el-table-column prop="position" min-width="100" label="收费日期">
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      crumbsData: {  //面包屑
        titleList: [
          { title: '账务中心', path: '/Recover' },
          { title: '账务调整', method: () => { window.histroy.back() } },
          { title: '追缴费用', method: () => { window.histroy.back() } }
  ...
  mounted() {
    ...
    this.$changeTable(this, '.Recover', ['.Recover .toolbar', '.Recover .block', '.Recover #crumbs'])
  },
  methods: {
    init() {
